...
def R(y_true, y_pred):
  ...
  numerator = tf.reduce_mean(tf.multiply(centered_y, centered_y_pred))
  denominator_1 = tf.sqrt(tf.reduce_mean(tf.square(centered_y)))
  denominator_2 = tf.sqrt(tf.reduce_mean(tf.square(centered_y_pred)))
