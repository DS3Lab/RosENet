...
if [ $# -eq 1 ]; then
  rm -f "/cluster/scratch/hhussein/Dataset/PDBBind/$1/score.sc"
  rm -f "/cluster/scratch/hhussein/Dataset/PDBBind/$1/processed"
  rm -f "/cluster/scratch/hhussein/Dataset/PDBBind/$1/*_00*"
  bash -c "make_rosetta /cluster/scratch/hhussein/Dataset/PDBBind/$1/$1_complex.pdb"
else
find /cluster/scratch/hhussein/Dataset/PDBBind/ -name "*_complex.pdb" | shuf | parallel -j 24 -n 1 --ungroup  bash -c ": && make_rosetta {}"
fi
