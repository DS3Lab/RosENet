...
function make_ligand_mol2() {
  ...
  complex=$1
  name=$(basename "$1" .pdb)
  mol="$dir/${name:0:4}_ligand_renamed.mol2"
  out=$dir/${name/complex/ligand}.mol2
  python2 $script $mol $complex > $dir/${name/complex/ligand}.mol2
}
export -f make_ligand_mol2

find /cluster/scratch/hhussein/Structures/set1 -name "*_complex_*.pdb" -maxdepth 2 | parallel -n 1 bash -c ": && make_ligand_mol2 {}"
find /cluster/scratch/hhussein/Structures/set2 -name "*_complex_*.pdb" -maxdepth 2 | parallel -n 1 bash -c ": && make_ligand_mol2 {}"
