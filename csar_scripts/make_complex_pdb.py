...
def make_complex_pdb(protein_pdb, ligand_pdb, complex_pdb):
  ...
  prot.filter("protein")
  lig = complex.copy()
  lig.filter("not protein and same residue as ((resname WAT and within 3 of resname WER and within 3 of protein) or (resname MN MG ZN CA NA and within 5 of resname WER) or resname WER)")
...
if __name__ == "__main__":
  parent_folder = Path('/cluster/scratch/hhussein/Structures/set1')
  #make_complex_pdb(*params(parent_folder, str(sys.argv[1])))
  p = Pool(48)
  p.map(process, get_files(parent_folder))
  parent_folder = Path('/cluster/scratch/hhussein/Structures/set2')
  p.map(process, get_files(parent_folder))
