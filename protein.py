...
class Protein:
    ...
    def _detect_structures(self):
        ...
        self.structures = [ProteinStructure(x) for x in structures_folder.iterdir() if x.is_dir()]
    ...
    def compute_grids(self, interpolation, parallel=False):
        ...
        if parallel:
            rmsf = self.rmsf
            pool = Pool(10)
            r = pool.map_async(func, zip(self.structures,itertools.repeat(rmsf), itertools.repeat(interpolation)))
            r.get()
    ...
    def _compute_rmsf(self):
        ...
        self.atoms = np.char.add(np.char.mod('%d-', self.atom_res_idxs), self.atom_names)
...
class ProteinStructure:
    ...
    def _detect_poses(self):
        self.poses = [ProteinPose(pose_filename)
                      for pose_filename in
                      self.path.glob(f'''{self.protein_name}_complex_{self.id}_{'[0-9]' * 4}.pdb''')]
...
class ProteinPose:
    _FILENAME_PATTERN = '([^_]+)_complex_([^_]+)_(\d{4})'

    def __init__(self, pdb_path):
        super(ProteinPose, self).__init__()
        self.grid_size = int(Config.config['Grid Parameters']['GridSize'])
        self.grid_spacing = float(Config.config['Grid Parameters']['GridSpacing'])
        self.grid_radius = float(Config.config['Grid Parameters']['GridRadius'])
        self.rmsf_window = int(np.floor(2 * self.grid_radius / self.grid_spacing))
        self.pdb_path = pdb_path
        self.protein_name, self.structure_id, self.id = re.match(ProteinPose._FILENAME_PATTERN, pdb_path.stem).groups()
        self.path = pdb_path.parent
        self.charges_path = self.path / f'{self.protein_name}_charges_{self.id}.db'
        self.radii_path = self.path / f'{self.protein_name}_radii_{self.id}.db'
        self.energies_path = self.path / f'{self.protein_name}_energies_{self.id}.db'
        self.charges = None
        self.radii = None
        self.energies = None
        self.grid = None
    ...
    def save_grids(self, interpolation):
        ...
        saving_path = self.path.parents[1] / 'images' / f'{self.protein_name}_{self.structure_id}_{self.id}_{interpolation}.pkl'
    ...
    def _import_energies(self):
        ...
        try:
            ...
            print("Error while connecting to the database " + self.energies_path)
            return -1
        cursor = conn.cursor()
    ...
    def _build_images(self, rmsf, interpolation):
        ...
        # orient_inibitor()
        protein_potential, ligand_potential = self._compute_electro_potential(protein_complex)
        ...
        selected_atoms = protein_complex.select(f'within {size_ang} of resname INH')
        ...
        mask = np.all(np.logical_and(coordinates >= ex_min, coordinates <= ex_max), axis=1)
        ...
        protein_res = np.unique(protein_complex.select('protein').getResindices() + 1)
        ...
        ccccc = 0
        for key_1 in keys:
            if not key_1 in self.energy_pairs:
                continue
            print(ccccc); ccccc += 1
            aa1 = np.array(np.meshgrid(x-coordinates[key_map[key_1]][0],y - coordinates[key_map[key_1]][1], z - coordinates[key_map[key_1]][2])).T
            r1 = np.linalg.norm(aa1, axis=3)
            prev_aux = (r1 ** (1 / self.radii[key_1] - 3)) * (np.exp(-2 * r1 / self.radii[key_1])) ** 2
            for key_2 in self.energy_pairs[key_1]:
                ...
                if r1 in protein_res and r2 in protein_res and np.abs(r1 - r2) == 1:
                    continue
                arr_atr[key_map[key_1]] += fa_atr
                #arr_atr[key_map[key_2]] += fa_atr
                arr_rep[key_map[key_1]] += fa_rep
                #arr_rep[key_map[key_2]] += fa_rep
                arr_sol[key_map[key_1]] += fa_sol
                #arr_sol[key_map[key_2]] += fa_sol
                arr_elec[key_map[key_1]] += fa_elec
                #arr_elec[key_map[key_2]] += fa_elec
                energy_idxs.add(key_map[key_1])
                #energy_idxs.add(key_map[key_2])
                if interpolation == 'pointwise_angular':
                    v = coordinates[key_map[key_2]] - coordinates[key_map[key_1]]
                    ...
                    filt = np.prod(np.dot(aa1,rot_matrix1.T), axis=3) * prev_aux
                    for field, values in {'fa_atr': fa_atr, 'fa_rep': fa_rep, 'fa_sol': fa_sol,
                                          'fa_elec': fa_elec}.items():
        ...
        aux_x, aux_y, aux_z = np.meshgrid(np.linspace(0, self.grid_size - 1, num=self.grid_size),
                                          ...
                                          np.linspace(0, self.grid_size - 1, num=self.grid_size))
        grid_points = np.array([aux_x.flatten(), aux_y.flatten(), aux_z.flatten()]).T
        ...
        if interpolation_class == 'pointwise' and mode is not 'angular':
            aa = np.array(np.meshgrid(x, y, z)).T
            if mode == 'exp_vdw':
                filter = lambda i : (1 - np.exp(-((radii[i] / np.linalg.norm(aa - coordinates[energy_idxs[i]], axis=3)) ** 12)))
            elif mode == 'gaussian':
                filter = lambda i: np.exp(
                    -0.5 * ((np.linalg.norm(aa - coordinates[energy_idxs[i]], axis=3) / radii[i]) ** 2))
            for i in range(len(energy_idxs)):
                aa1 = filter(i)
                for field, values in {'fa_atr': arr_atr, 'fa_rep': arr_rep, 'fa_sol': arr_sol,
                                      'fa_elec': arr_elec}.items():
                    image[field] = np.max(image[field], values[i] * aa1)
        else:
            for field, values in {'fa_atr': arr_atr, 'fa_rep': arr_rep, 'fa_sol': arr_sol, 'fa_elec': arr_elec}.items():
    ...
    def _compute_electro_potential(self, pose_pdb):
        ...
        center = np.array2string(calcCenter(pose_pdb.select('resname INH').getCoords()))[1:-1]
        ...
        ligand_potential = APBS.run(self.path, 'ligand', pose_pdb,
