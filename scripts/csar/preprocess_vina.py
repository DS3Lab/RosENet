...
def get_files(folder):
  return ([os.path.join(folder, x) for x in os.listdir(folder) if fnmatch(x, '*_protein*.pdb')],
          [os.path.join(folder, x) for x in os.listdir(folder) if fnmatch(x, '*_ligand*.mol2')])
