...
def get_files(folder):
  return ([os.path.join(folder, x) for x in os.listdir(folder) if fnmatch(x, '*_protein_*.pdb') and not x.endswith('autopsf.pdb')],
          [os.path.join(folder, x) for x in os.listdir(folder) if fnmatch(x, '*_ligand_*.mol2') and not x.endswith('autopsf.mol2')])
