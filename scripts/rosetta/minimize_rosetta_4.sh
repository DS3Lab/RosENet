...
function make_rosetta() {
  ...
  if [ ! -f "processed" ]; then
    ...
    complex=$(basename "$1") name=$(basename "$1" .pdb) params="${name/complex/ligand}.params" envsubst < $script_folder/flags_relax_2.txt > "flags_relax.txt"
    touch "processed"
    ($rosetta @flags_relax.txt -parser:protocol  "$script_folder/relax_4.xml" -database $rosettadb) && echo "Finished $(basename $folder) correctly"
...
find /cluster/scratch/hhussein/pdbbind2018/ -name "*_complex.pdb" | parallel -j 24 -n 1 --ungroup  bash -c ": && make_rosetta {}"
fi
