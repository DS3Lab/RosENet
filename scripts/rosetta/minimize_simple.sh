#!/bin/bash
rosetta=/cluster/scratch/hhussein/rosetta_bin_linux_2018.33.60351_bundle/main/source/bin/minimize.static.linuxgccrelease
...
function make_rosetta() {
  ...
  if [ ! -f "processed" ]; then
    ...
    touch "processed"
    ($rosetta @flags_relax.txt -database $rosettadb) && echo "Finished $(basename $folder) correctly"
